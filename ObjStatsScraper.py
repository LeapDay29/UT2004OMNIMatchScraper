...
fm_df.to_csv("objbigmatches.csv", index = False)
print("Saved objbigmatches.csv. Ctrl + click the timestamp to view the raw statistics (opens the match's UTStatsDB page in your browser).")

# -------------------------------------------------------------- #
## PULL THE OBJECTIVE-BASED PLAYER STATISTICS OF ALL THE SELECTED BIG MATCHES ##

# Load the new CSV file in
md = pd.read_csv("objbigmatches.csv")
...
# Perform the scrape of ONLY the Onslaught Summary statistics contained within qualifying match pages
...
if all_tables:
    ...
    playerstats.to_csv("objstatsraw.csv", index=False)
    print("Saved all player stats for the indicated matches to objstatsraw.csv")
else:
...
ps = pd.read_csv("objstatsraw.csv")
print(ps.head(10))

# Trim off redundant/erroneous rows/columns. Preview it again to see the changes
ps = ps.iloc[2:]
ps = ps[~ps["0"].str.contains("Team", case=False, na=False)]
ps = ps[~ps["0"].str.contains("Onslaught", case=False, na=False)]
ps = ps.reset_index(drop=True)
ps = ps.drop(columns=["6"])
print(ps.head(50))
...
psabc.columns = ["RankInMatch", "PlayerName", "FinalScore", "NodesBuilt", "NodesDestr", "WIPNodesDestr"]
print(psabc.head(10))
...
psabc.to_csv("objstats.csv", index=False)
